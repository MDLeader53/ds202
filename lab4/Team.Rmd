...
head(offense)
offClean <- offense
offClean$Receiving_REC <-  as.numeric(offense$Receiving_REC)
offClean$Receiving_YDS <-  as.numeric(offense$Receiving_YDS)
offClean$Receiving_TD <-  as.numeric(offense$Receiving_TD)
offClean$Receiving_ATT <-  as.numeric(offense$Receiving_ATT)
offClean$Rushing_TD <-  as.numeric(offense$Rushing_TD)
offClean$`Passing_CMP-ATT` <-  as.numeric(offense$`Passing_CMP-ATT`)
offClean$Passing_YDS <-  as.numeric(offense$Passing_YDS)
offClean$Passing_TD <-  as.numeric(offense$Passing_TD)
offClean$Passing_INT <-  as.numeric(offense$Passing_INT)
```
...
```{r,echo=FALSE,results='hide'}
defClean <- defClean %<% gather(key = Statistic, value = cases, 2:5)
```
...
```{r,echo=FALSE,results='hide'}
ggplot2(defClean, axis(x='Statistic', y='Name')) + geom_point()
```
...
```{r,echo=FALSE,results='hide'}
# strsplit only works for input type character vector
y =  bioClean %>% as.character() %>%  strsplit(bioClean[6],"," )
#y is a char vector
print(y)
head(bioClean)
```
